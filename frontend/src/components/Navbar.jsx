  ...
  return (
    <>
      <aside className="app-sidebar snx-sidebar-shell">
        <div className="snx-mobile-nav-head">
          <Link to="/dashboard" className="text-decoration-none" onClick={() => setMobileMenuOpen(false)}>
            <Brand />
          </Link>
          <button
            type="button"
            className="btn snx-mobile-nav-toggle"
        ...
        <div className={`snx-mobile-nav-panel ${mobileMenuOpen ? "open" : ""}`}>
            ...
            <div className="snx-nav-list">
              {navItems.map((item) => (
                <NavLink
                  key={item.path}
                  to={item.path}
                  className={({ isActive }) => `snx-nav-link ${isActive ? "active" : ""}`}
